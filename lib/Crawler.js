...
Crawler.prototype.start = function() {
  ...
  phantom.create(function(ph) {
    ...
    var interval = setInterval(function(){
      logger.routine(crawler.queue.length +' tasks in queue; ' +
        crawler.dynamicTaskQueue.length+ ' dynamic page tasks in queue; ' +
        crawler.running + ' tasks running');

      if (crawler.queue.length <= 0) {
        if (lastCheckPoint == crawler.checkPoint){
          logger.routine('no active task in ' + crawler.endCheckInterval/1000 + ' seconds');
          crawler.ph.exit();
