...
Task.prototype.run = function() {
  console.log('[Base Task] | running task: ' + this.url);
  var task = this;
  ...
  if (task.context.dynamicpage == true) {
    console.log('[Base Task] | start crawling dynamic page');
    this.crawler.ph.createPage(function(page) {
      return page.open(task.url, function(status) {
        ...
        if (status == "success") {
          ...
          // failed to load page
          console.error('[Base Task] failed to load page: ' + task.url);
        }
    ...
    if (task.method == 'post') {
      console.log('[Base Task] | send POST request');
      var r = request.post({
        'uri' : task.url,
...
//        var url = task.url;
//        if ( typeof response.request != 'undefined' && response.request != null ){
...
//        }
      ...
      console.log('[Base Task] | send GET request');
